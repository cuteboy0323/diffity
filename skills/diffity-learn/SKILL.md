...
5. **Start a Diffity tree instance** for the learning directory if one isn't already running. Run `diffity tree --no-open` from the learning directory using Bash with `run_in_background: true`. Wait 2 seconds, then verify with `diffity list --json`.
